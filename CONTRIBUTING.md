# Contributing to Headroom Token View
...
```bash
git clone https://github.com/chopratejas/headroom-token-view.git
cd headroom-token-view
...
3. **Smoke** — `.venv/bin/htv start`, send a real call through, refresh the dashboard, `htv stop`.
...
│   ├── logger.py                # HtvLogger CustomLogger — the heart of cost capture
│   ├── pubsub.py                # In-process async fan-out for SSE
│   ├── config.py                # Pydantic config + ~/.headroom-token-view/config.yaml
...
| Want to... | Edit... |
|---|---|
| Add a new dashboard endpoint | `src/headroom_token_view/dashboard.py` |
| Add a new SQL aggregate | `src/headroom_token_view/db.py` |
| Change how a provider's usage is mapped to a row | `src/headroom_token_view/logger.py::HtvLogger._build_row` + a test in `tests/test_logger.py` |
| Add a CLI command | `src/headroom_token_view/cli.py` |
| Add a config knob | `src/headroom_token_view/config.py` (Pydantic model) |
...
- **Tests**: synthetic payloads, not network. If a test needs a real provider, gate it on `HTV_E2E=1` so CI doesn't burn quota.
...
This is the most load-bearing part of HTV. If you're changing how usage flows from LiteLLM into our rows:
...
Use the GitHub issue templates. For bugs in cost calculation specifically, include the **provider**, **model**, **stream vs non-stream**, the `usage` object from the response (redact prompts), and the cost HTV recorded.
